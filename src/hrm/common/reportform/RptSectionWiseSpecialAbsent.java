...
public class RptSectionWiseSpecialAbsent extends Window 
{
	...
	public RptSectionWiseSpecialAbsent(SessionBean sessionBean) 
	{
		...
		this.setCaption("DAILY SPECIAL ABSENT (Section Wise) :: "+sessionBean.getCompany());
	...
	private void reportShow(Object sectionId)
	{
		...
		try
		{
			...
			/*query = " select vEmployeeId,vEmployeeName,vDesignation,vSectionName,dAttInTime,dAttOutTime, "+
					" SUBSTRING(CONVERT(varchar,(CONVERT(time,( "+
					" convert(varchar(5),DateDiff(s, dAttInTime,dAttOutTime)/3600)+':'+ "+
					" convert(varchar(5),DateDiff(s, dAttInTime,dAttOutTime)%3600/60)+':'+ "+
					" convert(varchar(5),(DateDiff(s, dAttInTime,dAttOutTime)%60)))))),1,8) totalTime "+
					" from tbEmployeeAttendance where vSectionId like '"+sectionId+"'" +
					" and dAttDate = '"+dfYMD.format(dDate.getValue())+"' ";*/
			
			/*query="select * from funCalcEmployeeAttendance('"+dfYMD.format(dDate.getValue())+"','"+dfYMD.format(dDate.getValue())+"','%','"+sectionId+"')" +
					" where (ISNULL(inTime,'')!='' or ISNULL(outTime,'')!='') and not (ISNULL(inTime,'')!='' and ISNULL(outTime,'')!='')";*/

			query="SELECT vFigId,vEmployeeName,vDeginationName,sectionIdDb,sectionName,dAttDate,(case when tIntime!='1900-01-01 00:00:00.000' " +
					"then tIntime end)as tIntime,(case when tOutTime!='1900-01-01 00:00:00.000' then tOutTime end)as tOutTime from " +
					"[funAttendanceReport] ('"+dfYMD.format(dDate.getValue())+"','"+sectionId+"','%') where (ISNULL(tIntime,'')!='1900-01-01 00:00:00.000' or " +
					"ISNULL(tOutTime,'')!='1900-01-01 00:00:00.000') " +
					"and not (ISNULL(tIntime,'')!='1900-01-01 00:00:00.000' and ISNULL(tOutTime,'')!='1900-01-01 00:00:00.000')";
