...
public class RptAgeAsOnDate extends Window 
{
	...
	public RptAgeAsOnDate(SessionBean sessionBean,String menuId)
	{
		this.sessionBean=sessionBean;
		this.setCaption("EMPLOYEE AGE AS ON DATE :: "+sessionBean.getCompany());
	...
	private void reportShow(){
		...
		try{
			...
			hm.put("SysDate",reportTime.getTime);
			hm.put("asOnDate", sessionBean.dfBd.format(dAsOnDate.getValue()));
			...
			query  = "select * from funJobDuration('"+unitId+"','"+deptId+"','"+secId+"','"+empType+"','"+empId+"','"+stAIctive+"','DoB','"+sessionBean.dfDb.format(dAsOnDate.getValue())+"')  order by vUnitName,vDepartmentName,vSectionName,iRank,dJoiningDate";

			report="report/account/hrmModule/RptAgeAsOnDate.jasper";
	...
	public AbsoluteLayout buildMainLayout()
	{
		...
		mainLayout.addComponent(chkEmployeeAll,"top:170px; left:345px;");
		...
		txtNumberOfYear.setVisible(false);
		...
		opgJoinConfirm.setVisible(false);

		lblDate=new Label("Till as on : ");
		mainLayout.addComponent(lblDate,"top:200px; left:30.0px;");
		...
		mainLayout.addComponent(dAsOnDate, "top:198px; left:140.0px;");
		...
		mainLayout.addComponent(new Label("Service Status :"),"top:230px; left:30px");
		mainLayout.addComponent(opgActivity, "top:228px; left:140.0px;");
