...
public class RptPFLoanRegister extends Window 
{
	...
	public RptPFLoanRegister(SessionBean sessionBean,String menuId) 
	{
		this.sessionBean=sessionBean;
		this.setCaption("PF LOAN SUMMARY :: "+sessionBean.getCompany());
		this.setWidth("500px");
	...
	private void cmbAddDepartmentName()
	{
		...
		try
		{
			String sql = " select distinct epo.vDepartmentId,epo.vDepartmentName from tbPFLoanApplication la inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=la.vAutoEmployeeId where epo.vUnitId='"+cmbUnit.getValue()+"' order by epo.vDepartmentName";
	...
	private void cmbAddSectionName()
	{
		...
		try
		{
			String sql = " select distinct epo.vSectionid,epo.vSectionname from tbPFLoanApplication la inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=la.vAutoEmployeeId where epo.vUnitId='"+cmbUnit.getValue()+"' and epo.vDepartmentId like '"+(chkDepartmentAll.booleanValue()==true?"%":cmbDepartment.getValue()==null?"%":cmbDepartment.getValue())+"' order by epo.vSectionName";
			List <?> list = session.createSQLQuery(sql).list();	
			for (Iterator <?> iter = list.iterator(); iter.hasNext();)
			{
				Object[] element =  (Object[]) iter.next();	
		...
		try
		{
			...
			if(radioLoanStatus.getValue().toString().equals("Active"))
			{
				query = " select ei.vEmployeeId,ei.vEmployeeCode,ei.vEmployeeName,si.vSectionName,di.vDesignation,ei.dJoiningDate," +
						...
						" from tbPFLoanApplication as la" +
			...
			if(queryValueCheck(query))
			{
				...
				Window win = new ReportViewer(hm,"report/account/hrmModule/rptPFLoanRegister.jasper",
