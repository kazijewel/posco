...
public class RptOverTimeStatement extends Window {
	...
	private static final List<String> accountType=Arrays.asList(new String[]{"Bank A/C","BFTN"});
	private CommonMethod cm;
	private String menuId = "";
	private OptionGroup RadioBtnGroup;
	private static final List<String> type1=Arrays.asList(new String[]{"PDF","Other","Excel"});
	String getAccount="";
	public RptOverTimeStatement(SessionBean sessionBean,String menuId)
	{
		this.sessionBean=sessionBean;
		this.setCaption("OVER TIME ADVICE :: "+sessionBean.getCompany());
		...
		try
		{
			if(opgReportType.getValue().toString().equals("Excel"))
			{
				...
				String signature[]={""};
						...
						exelSql="select vAccountNo,vDrCr,CONVERT(float,mAmount)Amount,vCurrency,vHomeBrn,vNarration from funEncloserInExcelOverTime('"+cmbMonthName.getValue().toString()+"','"+bankId+"','"+opgMoneyTransferType.getValue().toString()+"','"+txtAccountNo.getValue()+"')";
				...
				int countInd=0;
					...
					{
					 Object [] element = (Object[])iter.next();	
					 detailQuery[countInd]="select vAccountNo,vDrCr,CONVERT(float,mAmount)mAmount,vCurrency,vHomeBrn,vNarration from funEncloserInExcelOverTime('"+cmbMonthName.getValue().toString()+"','"+bankId+"','"+opgMoneyTransferType.getValue().toString()+"','"+txtAccountNo.getValue()+"')";
			...
			else
			{
				if(opgReportType.getValue().toString().equals("Forwarding Letter"))
				{
					...
					query="select vBankId,CONVERT(Decimal(18,0),mTotalGrossAmount)mAmount,(vTotalGrossAmountInWord)vAmountInWords,"+
							" vMoneyTransferType,(CONVERT(varchar,vMonthName)+'-'+CONVERT(varchar,vYear))vSalaryOfMonth,iEmployee "+
							" from funBanckAdviseMonthWiseOtStatement('"+cmbMonthName.getValue().toString()+"','%','%','%',"+
							" '"+bankId+"','"+opgMoneyTransferType.getValue().toString()+"')";
					...
					if(opgMoneyTransferType.getValue().toString().equals("Bank A/C"))
					{
						rptName="rptBankForwardingLetterOverTime.jasper";
					}
					else if(opgMoneyTransferType.getValue().toString().equals("BFTN"))
					{
						rptName="rptBankForwardingLetterBFTNOverTime.jasper";
					}
					System.out.println(2);
				}
				else if(opgReportType.getValue().toString().equals("Encloser"))
				{
					...
					query="select vEmployeeId,vEmployeeCode,vEmployeeName,vAccountNo,vBankId,CONVERT(Decimal(18,0),totalAmount)mAmount,(vTotalGrossAmountInWord)vAmountInWords,"+
							" vMoneyTransferType,(CONVERT(varchar,vMonthName)+'-'+CONVERT(varchar,vYear))vSalaryOfMonth,iEmployee "+
							" from funBanckAdviseMonthWiseOtStatement('"+cmbMonthName.getValue().toString()+"','%','%','%',"+
							" '"+bankId+"','"+opgMoneyTransferType.getValue().toString()+"')";
					...
					rptName="RptBankAdviceOverTimeStatement.jasper";
