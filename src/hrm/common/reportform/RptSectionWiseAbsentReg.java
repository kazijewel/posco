...
public class RptSectionWiseAbsentReg extends Window {
	...
	public RptSectionWiseAbsentReg(SessionBean sessionBean)
	{
		this.sessionBean=sessionBean;
		this.setCaption("DIVISION WISE MANUALLY ABSENT REGISTER :: "+sessionBean.getCompany());
	...
	private void reportShow()
	{
		...
		try
		{
			...
			query="select ap.vEmployeeID,ei.iFingerID,ei.vProximityID,ei.vEmployeeName,(select di.designationName " +
					"from tbDesignationInfo di where ap.vDesignationID=di.designationId) designation," +
					"(select si.SectionName from tbSectionInfo si where ap.vSectionID=si.AutoID) Section ," +
					"ap.dDate,ap.vReason,ap.userName,ap.userIP,ap.dEntryTime from tbAbsentAsPunishment ap " +
					"inner join tbEmployeeInfo ei on ei.employeeCode=ap.vEmployeeID where ap.vSectionID like '"+section+"'" +
					"order by ap.vDesignationID,ap.vEmployeeID,ap.dDate  ";
			...
			if(queryValueCheck(query))
			{
				...
				Window win = new ReportViewer(hm,"report/account/hrmModule/rptManuallyAbsentRegister.jasper",
