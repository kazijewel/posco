...
public class RptMonthWiseOtStatement extends Window
{
	...
	public RptMonthWiseOtStatement(SessionBean sessionBean,String menuId)
	{
		this.sessionBean=sessionBean;
		this.setCaption("MONTH WISE OT STATEMENT :: "+sessionBean.getCompany());
	...
	public void cmbDepartmentAddData()
	{
		...
		try
		{
			String query="select distinct  vDepartmentID,vDepartmentName from tbMonthlySalary "
					+ "where vSalaryMonth=DATENAME(MM,'"+dFormat.format(dSalaryMonth.getValue())+"') "
	...
	public void cmbSectionAddData()
	{
		...
		try
		{
			String query="select distinct  vSectionID,vSectionName from tbMonthlySalary "
					+ "where vSalaryMonth=DATENAME(MM,'"+dFormat.format(dSalaryMonth.getValue())+"') "
					+ " and vUnitId='"+cmbUnit.getValue().toString()+"' and vDepartmentId like '"+(chkDepartmentAll.booleanValue()?"%":cmbDepartment.getValue()==null?"%":cmbDepartment.getValue())+"' order by vSectionName";
	...
	public void employeeSetData()
	{
		...
		try
		{
			String query = "select vEmployeeId,vEmployeeCode,vEmployeeName,vProximityId from tbMonthlySalary " +
					"where vUnitId='"+cmbUnit.getValue().toString()+"' and vSectionId like '"+sectionId+"' " +
	...
	private void reportShow()
	{
		...
		try
		{			
			String query="select * from funMonthWiseOtStatement ('"+dSalaryMonth.getValue()+"',"
			...
			if(queryValueCheck(query))
			{
				...
				hm.put("month",dSalaryMonth.getValue());
				hm.put("year",fYear.format(dSalaryMonth.getValue()));
				hm.put("SysDate",reportTime.getTime);
				hm.put("logo", sessionBean.getCompanyLogo());
				hm.put("sql", query);

				Window win = new ReportViewer(hm,"report/account/hrmModule/RptMonthWiseOtStatement.jasper",
