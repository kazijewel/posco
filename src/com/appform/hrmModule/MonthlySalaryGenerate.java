...
public class MonthlySalaryGenerate extends Window
{
	...
	public MonthlySalaryGenerate(SessionBean sessionBean,String menuId) 
	{
		this.sessionBean= sessionBean;
		this.setCaption("GENERATE MONTHLY SALARY :: " +sessionBean.getCompany());
	...
	private void generateAction()
	{
		...
		String empQuery = "select vEmployeeId,vEmployeeName from tbEmpOfficialPersonalInfo where vUnitId='"+cmbUnit.getValue().toString()+"' and vSectionId like '"+section+"' and vDepartmentId like '"+(cmbDepartment.getValue()==null?"%":cmbDepartment.getValue())+"' "
		...
		{
			String query = "select * from tbMonthlySalary where MONTH(dSalaryDate) = MONTH('"+dFormat.format(dSalaryMonth.getValue())+"') "
					+ "AND YEAR(dSalaryDate) = YEAR('"+dFormat.format(dSalaryMonth.getValue())+"') AND"
							+ " vUnitId ='"+cmbUnit.getValue().toString()+"' and vSectionId like '"+section+"' and vEmployeeID "
									+ "like '"+employee+"'";
		...
		{
			showNotification("There are no Employee in this Division",Notification.TYPE_WARNING_MESSAGE);
			txtClear();
			componentIni(true);
		}
	}
...
*/		
	...
	private void salaryGenerate()
	{
		...
		try
		{
			String insertQuery = "exec prcCalcMonthlySalary '"+dFormat.format(dGenerateDate.getValue())+"','"+dFormat.format(dSalaryMonth.getValue())+"','"+employee+"','"+cmbUnit.getValue().toString()+"','"+section+"','"+txtRevenueStamp.getValue()+"','"+sessionBean.getUserName()+"','"+sessionBean.getUserIp()+"'";
			
			session.createSQLQuery(insertQuery).executeUpdate();
			
			System.out.println(insertQuery);
			Notification n=new Notification("","Salary Generated Successfully!",Notification.TYPE_HUMANIZED_MESSAGE);
