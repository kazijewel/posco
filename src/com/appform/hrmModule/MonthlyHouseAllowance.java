			...
			if(!list.isEmpty())
			{
				if(list.iterator().hasNext())
				{
					...
					for(int i=0; i<list.size(); i++)
					{
						...
						txtHouseAll.get(i).setValue(element[7]);
					}
	...
	private void updateData()
	{
		...
		try
		{
			for(int i = 0;i<lblEmployeeId.size();i++)
			{
				if(!lblEmployeeId.get(i).getValue().toString().isEmpty())
				{
					String udquery="insert into tbMonthlyHouseAllowance(" +
							"dDate,vEmployeeID,vEmployeeCode,vEmployeeName,vDesignationName,dJoiningDate,vUnitId,vUnitName,vDepartmentId,"
							+ "vDepartmentName,vSectionID,vSectionName,mHouseRent,vUserId,vUserName,vUserIP,dEntryTime" +
							") "
							+ "values ()";
					...
					String query = "update tbEmpOfficialPersonalInfo " +
							"set mHouseRent='"+(txtHouseAll.get(i).getValue().toString().trim().isEmpty()?0:txtHouseAll.get(i).getValue().toString().trim())+"'," + 
