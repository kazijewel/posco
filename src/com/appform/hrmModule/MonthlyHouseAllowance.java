...
public class MonthlyHouseAllowance extends Window 
{
	...
	private ArrayList<TextField> txtHouseAll = new ArrayList<TextField>();
	...
	public MonthlyHouseAllowance(SessionBean sessionBean,String menuId) 
	{
		this.sessionBean=sessionBean;
		this.setResizable(false);
		cm = new CommonMethod(sessionBean);
		this.menuId = menuId;
		this.setCaption("MONTHLY HOUSE ALLAWANCE :: " + sessionBean.getCompany());
		buildMainLayout();
	...
	{
		Window win = new MonthlyHouseAllowanceFind(sessionBean, findId, empID);
	...
	private void findInitialize(String findId, String strEmpID)
	{
		...
		try
		{
			String sql = "select vEmployeeID,vEmployeeCode,vEmployeeName,vDesignationName,vDepartmentId,vDepartmentName,"
					+ "(select vServiceType from tbEmpOfficialPersonalInfo where vEmployeeID=tbMonthlyHouseAllowance.vEmployeeID)vServiceType, "
					+ "dJoiningDate,mHouseRent,dDate,vUnitId,vDepartmentId,vSectionID,vEmployeeID,iAutoID "
			...
			if(lst.iterator().hasNext())
			{
				...
				txtHouseAll.get(i).setValue(element[8]);
				i++;
	...
	private void addTableData()
	{
		...
		try
		{
			String check = " SELECT vEmployeeID from tbMonthlyHouseAllowance where MONTH(dDate)=MONTH('"+dFormat.format(dDate.getValue())+"') "
			...
			{				
				String sql = "select a.vEmployeeID,vEmployeeCode,a.vEmployeeName,vDesignationName,a.vDepartmentId,a.vDepartmentName,a.dJoiningDate,"
						+ "b.mHouseRent,vServiceType from tbEmpOfficialPersonalInfo a "
						...
						+ "and mHouseRent>0 and bStatus=1 "
						+ "order by SUBSTRING(vEmployeeCode,3,50) ";
				...
				for (Iterator <?> iter = lst.iterator(); iter.hasNext();) {
					...
					if(t)
					{
						...
						txtHouseAll.get(i).setValue(dfZero.format(element[7]));
			...
			for(int i = 0;i<lblEmployeeId.size();i++)
			{
				if(!lblEmployeeId.get(i).getValue().toString().isEmpty())
				{
					...
					String queryOld="insert into tbUdMonthlyHouseAllowance(" +
							...
							+ "from tbMonthlyHouseAllowance where iAutoID='"+txtAutoId.getValue()+"' ";

					System.out.println("queryOld :" +queryOld);					
					session.createSQLQuery(queryOld).executeUpdate();

					/*Delete Data*/
					String queryDelete="delete from tbMonthlyHouseAllowance where iAutoID='"+txtAutoId.getValue()+"' ";					
	...
	private void insertData()
	{
		...
		try
		{
			for(int i = 0;i<lblEmployeeId.size();i++)
			{
				if(!lblEmployeeId.get(i).getValue().toString().isEmpty())
				{
					String query="insert into tbMonthlyHouseAllowance(" +
							"dDate,vEmployeeID,vEmployeeCode,vEmployeeName,vDesignationName,dJoiningDate,vUnitId,vUnitName,vDepartmentId,"
							+ "vDepartmentName,vSectionID,vSectionName,mHouseRent,vEmployeeType,vUserId,vUserName,vUserIP,dEntryTime" +
							") "
							+ "values ("
			...
			for(int i = 0;i<lblEmployeeId.size();i++)
			{
				if(!lblEmployeeId.get(i).getValue().toString().isEmpty())
				{
					...
					String queryOld="insert into tbUdMonthlyHouseAllowance(" +
							...
							") "
							+ "select dDate,vEmployeeID,vEmployeeCode,vEmployeeName,vDesignationName,dJoiningDate,vUnitId,vUnitName,vDepartmentId,"
							+ "vDepartmentName,vSectionID,vSectionName,mHouseRent,vEmployeeType,'UPDATE',vUserId,vUserName,vUserIP,dEntryTime "
							+ "from tbMonthlyHouseAllowance where iAutoID='"+txtAutoId.getValue()+"' ";
					...
					/*Insert Data*/
					String queryUpdate="insert into tbMonthlyHouseAllowance(" +
							"dDate,vEmployeeID,vEmployeeCode,vEmployeeName,vDesignationName,dJoiningDate,vUnitId,vUnitName,vDepartmentId,"
							+ "vDepartmentName,vSectionID,vSectionName,mHouseRent,vEmployeeType,vUserId,vUserName,vUserIP,dEntryTime" +
							") "
							+ "values ("
	...
	private void tableRowAdd( final int ar)
	{
				...
				for(int rowcount=ar;rowcount<=lblEmployeeId.size()-1;rowcount++)
				{
					if(rowcount+1<=lblEmployeeId.size()-1)
					{
						if(!lblEmployeeId.get(rowcount+1).getValue().toString().equals(""))
						{
							...
							txtHouseAll.get(rowcount).setValue(txtHouseAll.get(rowcount+1).getValue().toString());
							...
							txtHouseAll.get(rowcount+1).setValue("");
						}
					}
				}
		...
		txtHouseAll.add(ar, new TextField());
		txtHouseAll.get(ar).setWidth("100%");
		txtHouseAll.get(ar).setImmediate(true);
