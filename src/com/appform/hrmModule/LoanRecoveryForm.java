...
public class LoanRecoveryForm extends Window
{
	...
	public LoanRecoveryForm(SessionBean sessionBean,String menuId)
	{
		this.sessionBean=sessionBean;
		this.setCaption("LOAN RECOVERY :: "+sessionBean.getCompany());
	...
	private void cmbUnitDataAdd() 
	{
		...
		try
		{						
			String query="select distinct epo.vUnitId,epo.vUnitName from tbLoanApplication la inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=la.vAutoEmployeeId order by epo.vUnitName";
	...
	private void cmbAddEmployeeData()
	{
		...
		try
		{
			String query = "select vAutoEmployeeId,epo.vFingerId,epo.vEmployeeCode,epo.vEmployeeName from tbLoanApplication la inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=la.vAutoEmployeeId where epo.vUnitId='"+cmbUnit.getValue()+"' and iLoanStatus = '1'";
	...
	private void findInitialise(String TranId, Object findDate) 
	{
		...
		try
		{
			String sql = "SELECT vAutoEmployeeId,vLoanNo," +
					"(select mLoanBalance from tbLoanApplication where vLoanNo=LRin.vLoanNo and vAutoEmployeeId=LRin.vAutoEmployeeId)+mRecoveryAmount," +
					...
					" from tbLoanRecoveryInfo LRin where vTransactionId = '"+TranId+"' and dRecoveryDate = '"+(findDate)+"' ";
	...
	private void employeeSetData(String employeeId)
	{
		...
		try
		{
			String query = "select vAutoEmployeeId,epo.vFingerId,epo.vEmployeeCode,epo.vEmployeeName,epo.vSectionName,"
					+ " epo.vDesignationName,epo.vDepartmentName from tbLoanApplication la "
	...
	private void employeeLoanData(String empId)
	{
		...
		try
		{
			String query = " select la.dApplicationDate,vLoanNo from tbLoanApplication la inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=la.vAutoEmployeeId where epo.vUnitId='"+cmbUnit.getValue().toString()+"' and vAutoEmployeeId='"+empId+"' and iLoanStatus='1' ";
	...
	private void setLoanData()
	{
		...
		try
		{
			String query = "SELECT mLoanBalance,mAmountPerInstall,iSanctionStatus,vLoanType from "
					+ " tbLoanApplication la inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=la.vEmployeeId where epo.vUnitId='"+cmbUnit.getValue().toString()+"' and "
	...
	private void insertData()
	{
		...
		try
		{
			insertQuery = " Insert into tbLoanRecoveryInfo (vTransactionId,dRecoveryDate,vAutoEmployeeId,vEmployeeId,vEmployeeName,vFingerId,vProximityId," +
