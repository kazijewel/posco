...
public class SectionWiseAttendanceManually extends Window
{
	...
	public SectionWiseAttendanceManually(SessionBean sessionBean)
	{
	...
	private void cmbSectionDataLoad()
	{
		...
		try
		{
			String sql="select distinct ei.vSectionId,se.SectionName from tbEmployeeInfo ei inner join " +
					"tbSectionInfo as se on ei.vSectionId=se.AutoID order by se.SectionName";
		...
		opgEmployee.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(cmbSection.getValue()!=null)
				{
					...
					{
						sql="select vEmployeeId,employeeCode from tbEmployeeInfo where  vSectionID like '"+SectionID+"' and ISNULL(vProximityId,'')!='' and iStatus=1 order by employeeCode";
					}
					else if(opgEmployee.getValue()=="Proximity ID")
					{
						sql="select vEmployeeId,vProximityId from tbEmployeeInfo where  vSectionID like '"+SectionID+"' and ISNULL(vProximityId,'')!='' and iStatus=1 order by employeeCode";
					}
					else if(opgEmployee.getValue()=="Employee Name")
					{
						sql="select vEmployeeId,vEmployeeName from tbEmployeeInfo where  vSectionID like '"+SectionID+"' and ISNULL(vProximityId,'')!='' and iStatus=1 order by employeeCode";
					}
					if(!sql.equals(""))
					{
						Session session=SessionFactoryUtil.getInstance().openSession();
	...
	private boolean checkExists()
	{
		...
		try
		{
			...
			for(int i=0;i<lblAutoEmployeeID.size();i++)
			{
				if(lblAutoEmployeeID.get(i).getValue().toString().length()>0)
				{
					...
					{
						ret = true;
						CHECKID += " "+lblEmployeeID.get(i).getValue().toString();
					}
				}
			}
		}
		...
		{
			showNotification("checkExists", exp.toString(), Notification.TYPE_WARNING_MESSAGE);
		}
	...
	private void saveButtonEvent()
	{
		...
		{
			showNotification("Warning", "Data already exists for"+CHECKID, Notification.TYPE_WARNING_MESSAGE);
		}
	}
				...
				if(!lblAutoEmployeeID.get(i).getValue().toString().isEmpty())
				{
					if(chkPermit.get(i).booleanValue())
					{
						...
						sqlManually="insert into tbAttendanceManually (vEmployeeID,vEmployeeCode,vProximityID,vEmployeeName," +
								...
								"EntryTime) values('"+lblAutoEmployeeID.get(i).getValue().toString()+"','"+lblEmployeeID.get(i).getValue().toString()+"','"+lblProximityID.get(i).getValue().toString()+"'," +
						...
						System.out.println("Shwe"+sqlManually);
	...
	private void tableValueAdd(String emp)
	{
		...
		try
		{
			...
			String sql="select ein.vEmployeeID,ein.employeeCode,ein.vProximityId,ein.vEmployeeName,ein.vdesignationId," +
					"din.designationName,ein.vSectionID,sein.sectionName,OtStatus" +
					" from tbEmployeeInfo ein inner join tbSectionInfo sein on ein.vSectionID=sein.Autoid " +
					"inner join tbDesignationInfo din on din.designationID=ein.vDesignationID and " +
	...
	public void tableRowAdd(final int ar)
	{
		...
		table.addItem(new Object[]{btnDel.get(ar),lblSl.get(ar),lblAutoEmployeeID.get(ar),lblEmployeeID.get(ar),lblProximityID.get(ar),lblEmployeeName.get(ar),
				lblDesignationID.get(ar),lblDesignation.get(ar),lblSectionID.get(ar),lblSectionName.get(ar),
	...
	public AbsoluteLayout buildMainLayout()
	{
		...
		mainlayout.addComponent(new Label("Section Name : "), "top:10.0px;left:30.0px;");
		mainlayout.addComponent(cmbSection, "top:08.0px;left:140.0px");
		...
		mainlayout.addComponent(opgEmployee, "top:35.0px;left:30.0px;");
		...
		mainlayout.addComponent(new Label("Employee : "), "top:60.0px;left:30.0px;");
		mainlayout.addComponent(cmbEmployee, "top:58.0px;left:120.0px;");

		chkAllEmp=new CheckBox("All");
		chkAllEmp.setImmediate(true);
		mainlayout.addComponent(chkAllEmp, "top:60.0px;left:415.0px");
		...
		mainlayout.addComponent(new Label("From Date : "), "top:10.0px;left:480.0px;");
		mainlayout.addComponent(fromdate, "top:08.0px;left:555.0px;");
		...
		mainlayout.addComponent(new Label("To Date : "), "top:35.0px;left:480.0px;");
		mainlayout.addComponent(todate, "top:33.0px;left:555.0px;");

		InHrs=new TimeField();
		InHrs.setWidth("25.0px");
		mainlayout.addComponent(new Label("In Time : "), "top:10.0px;left:675.0px;");
		mainlayout.addComponent(InHrs, "top:08.0px;left:740.0px;");
		mainlayout.addComponent(new Label(" : "),"top:08.0px;left:767.0px;");

		InMin=new TimeField();
		InMin.setWidth("25.0px");
		mainlayout.addComponent(InMin, "top:08.0px;left:771.0px;");

		InTF=new TextField();
		InTF.setWidth("28.0px");
		InTF.setInputPrompt("AM");
		InTF.setTextChangeEventMode(TextChangeEventMode.EAGER);
		mainlayout.addComponent(InTF, "top:08.0px;left:800.0px;");

		outHrs=new TimeField();
		outHrs.setWidth("25.0px");
		mainlayout.addComponent(new Label("Out Time : "), "top:35.0px;left:675.0px;");
		mainlayout.addComponent(outHrs, "top:33.0px;left:740.0px;");
		mainlayout.addComponent(new Label(" : "),"top:33.0px;left:767.0px;");

		outMin=new TimeField();
		outMin.setWidth("25.0px");
		mainlayout.addComponent(outMin, "top:33.0px;left:771.0px;");

		outTF=new TextField();
		outTF.setWidth("28.0px");
		outTF.setInputPrompt("PM");
		outTF.setTextChangeEventMode(TextChangeEventMode.EAGER);
		mainlayout.addComponent(outTF, "top:33.0px;left:800.0px;");

		chkPermitAll=new CheckBox("Apply All");
		chkPermitAll.setImmediate(true);
		mainlayout.addComponent(chkPermitAll, "top:35.0px;left:1045.0px");

		table.setWidth("895.0px");
		...
		mainlayout.addComponent(table, "top:95.0px;left:30.0px");
		tableinitialize();

		mainlayout.addComponent(cButton, "top:465.0px;left:300.0px;");
		return mainlayout;
	}
}
