...
public class LeaveApprovePrimary extends Window 
{
	...
	public LeaveApprovePrimary( SessionBean sessionBean,String menuId)
	{
		this.sessionBean=sessionBean;
		this.setCaption("LEAVE APPLICATION APPROVE PRIMARY :: "+sessionBean.getCompany());
	...
	private void cmbUnitDataAdd()
	{
		...
		try
		{
			...
			String query=" select epo.vUnitId,epo.vUnitName from tbEmpOfficialPersonalInfo epo inner join "
					+ " tbEmpLeaveApplicationInfo eli on epo.vEmployeeId=eli.vEmployeeId  where eli.iPrimary=0 and eli.iHR=0 and eli.iFinal=0 "
			...
			for(int i = 0; i<tbLblEmployeeName.size(); i++)
			{
				if(tbChkSelect.get(i).booleanValue())
				{
					...
					String updateDetails = " update tbEmpLeaveApplicationDetails set " +
							" iPrimary=1 where vLeaveId = '"+tbLblReference.get(i).getValue().toString()+"' ";
	...
	private void addTableData()
	{
		...
		try
		{
			String sql = "select vLeaveId,eli.dApplicationDate,epo.vEmployeeName,epo.vDepartmentName,epo.vDesignationName,dSanctionFrom,dSanctionTo,mTotalDays," 
					...
					+" and epo.vDepartmentId in (select vDepartmentId from tbLeaveApprovalMapping where vDesignationIdPrimary='"+sessionBean.getDesignationId()+"' ) "
					+" and iPrimary = 0  order by eli.dApplicationDate";
