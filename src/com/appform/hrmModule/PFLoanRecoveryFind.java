...
public class PFLoanRecoveryFind extends Window 
{
	...
	public PFLoanRecoveryFind(SessionBean sessionBean, TextRead txtTranId, TextRead txtFindDate)
	{		
		this.txtTranId = txtTranId;
		this.txtFindDate = txtFindDate;
		this.sessionBean = sessionBean;
		this.setCaption("FIND PF LOAN RECOVERY :: "+sessionBean.getCompany());
		this.center();
		this.setWidth("570px");
	...
	private void cmbUnitData()
	{
		...
		try
		{
			String query = "Select distinct epo.vUnitId,epo.vUnitName FROM tbPFLoanRecoveryInfo lri inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=lri.vAutoEmployeeId order by epo.vUnitName ";
	...
	private void cmbAddEmployeeData()
	{
		...
		try
		{
			String query = "Select distinct vAutoEmployeeId,epo.vEmployeeCode FROM tbPFLoanRecoveryInfo lri inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=lri.vAutoEmployeeId where "
	...
	private void findButtonEvent()
	{
		...
		try
		{
			String Findquery = "Select vTransactionId,dRecoveryDate,mRecoveryAmount from tbPFLoanRecoveryInfo" +
					" where "
	...
	private void employeeLoanData(String empId)
	{
		...
		try
		{
			String query = " select dRecoveryDate,vLoanNo from tbPFLoanRecoveryInfo lri inner join tbEmpOfficialPersonalInfo epo on epo.vEmployeeId=lri.vAutoEmployeeId where epo.vUnitId='"+cmbUnit.getValue().toString()+"' and " +
