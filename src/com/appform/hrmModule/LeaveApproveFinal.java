	...
	private void addTableData()
	{
		...
		try
		{
			String sql = "select vLeaveId,eli.dApplicationDate,epo.vEmployeeName,epo.vDepartmentName,epo.vDesignationName,dSanctionFrom,dSanctionTo,mTotalDays," 
					...
					+" and epo.vDepartmentId in (select vDepartmentId from tbLeaveApprovalMapping where vDesignationIdFinal='"+sessionBean.getDesignationId()+"' ) "
					+" and epo.vDepartmentId like '"+(cmbDepartment.getValue()==null?"%":cmbDepartment.getValue().toString())+"' and iFinal = 0 and iHR=1 and iPrimary=1  order by eli.dApplicationDate";
