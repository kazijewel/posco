...
public class LeaveApproveFinal extends Window 
{
	...
	public LeaveApproveFinal( SessionBean sessionBean,String menuId)
	{
		this.sessionBean=sessionBean;
		this.setCaption("LEAVE APPLICATION APPROVE FINAL :: "+sessionBean.getCompany());
	...
	String query=" select epo.vDepartmentId,epo.vDepartmentName from tbEmpOfficialPersonalInfo epo inner join tbEmpLeaveApplicationInfo "
			+ " eli on epo.vEmployeeId=eli.vEmployeeId  where epo.vUnitId='"+cmbUnit.getValue().toString()+"' and eli.iFinal =0 and eli.iHR=1 and eli.iPrimary=1 "
			...
			for(int i = 0; i<tbLblEmployeeName.size(); i++)
			{
				if(tbChkSelect.get(i).booleanValue())
				{
					...
					String updateDetails = " update tbEmpLeaveApplicationDetails set " +
							" iFinal=1,iApprovedFlag=1 where vLeaveId = '"+tbLblReference.get(i).getValue().toString()+"' ";
					...
					String leaveAttendanceSummary = "update tbAttendanceSummary set iLeaveWithPay = (select COUNT(dLeaveDate)"
							...
							+ " iFinal = 1 and iApprovedFlag=1 and vPaymentFlag !='Without Pay'), iLeaveWithoutPay = (select COUNT(dLeaveDate)"
							+ " from tbEmpLeaveApplicationDetails where vEmployeeID = '"+tbLblEmployeeId.get(i).getValue()+"' "
							+ " and MONTH(dLeaveDate) = MONTH('"+sessionBean.dfDb.format(tbdLeaveFrom.get(i).getValue())+"') and "
							+ " YEAR(dLeaveDate) = YEAR('"+sessionBean.dfDb.format(tbdLeaveFrom.get(i).getValue())+"') and "
							+ " iFinal = 1 and iApprovedFlag=1 and vPaymentFlag ='Without Pay') where vEmployeeID = '"+tbLblEmployeeId.get(i).getValue()+"' and "
	...
	private void addTableData()
	{
		...
		try
		{
			String sql = "select vLeaveId,eli.dApplicationDate,epo.vEmployeeName,epo.vDepartmentName,epo.vDesignationName,dSanctionFrom,dSanctionTo,mTotalDays," 
					...
					+" and epo.vDepartmentId in (select vDepartmentId from tbLeaveApprovalMapping where vDesignationIdFinal='"+sessionBean.getDesignationId()+"' ) "
					+" and iFinal = 0 and iHR=1 and iPrimary=1  order by eli.dApplicationDate";
