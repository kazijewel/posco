...
public class LeaveApproveHR extends Window 
{
	...
	public LeaveApproveHR( SessionBean sessionBean,String menuId)
	{
		this.sessionBean=sessionBean;
		this.setCaption("LEAVE APPLICATION APPROVE HR :: "+sessionBean.getCompany());
	...
	private void cmbDepartmentDataAdd()
	{
		...
		try
		{
			...
			+ " eli on epo.vEmployeeId=eli.vEmployeeId  where epo.vUnitId='"+cmbUnit.getValue().toString()+"' and eli.iPrimary =1 and eli.iHR=0 and eli.iFinal=0 "
			+ " and epo.vDepartmentId in (select vDepartmentId from tbLeaveApprovalMapping where vDesignationIdHR='"+sessionBean.getDesignationId()+"' ) "
			...
			for(int i = 0; i<tbLblEmployeeName.size(); i++)
			{
				if(tbChkSelect.get(i).booleanValue())
				{
					...
					String updateDetails = " update tbEmpLeaveApplicationDetails set " +
							" iHR=1 where vLeaveId = '"+tbLblReference.get(i).getValue().toString()+"' ";
