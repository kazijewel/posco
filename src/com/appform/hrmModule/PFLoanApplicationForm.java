...
public class PFLoanApplicationForm extends Window
{
	...
	public PFLoanApplicationForm(SessionBean sessionBean,boolean isPreview,String menuId)
	{
	...
	{
		Window win = new PFLoanAppFind(sessionBean, txtFindLoanNo, txtFindDate);
	...
	private Object autoId()
	{
		...
		try 
		{
			String query = " Select cast(isnull(max(cast(replace(vLoanNo, '', '')as int))+1, 1)as varchar) as loanId from tbPFLoanApplication ";
	...
	private void findInitialise(String LoanId, Object findDate) 
	{
		...
		try 
		{
			String sql="select iAutoId,vLoanNo,dApplicationDate,vAutoEmployeeId,vLoanType,mLoanAmount,mInterestRate, " +
					...
					"from tbPFLoanApplication la where vLoanNo = '"+LoanId+"' and dApplicationDate = '"+findDate+"' ";
	...
	private void employeeSetData(String employeeId)
	{
		...
		List <?> list = session.createSQLQuery(query).list();
			...
			String pending = " select * from tbPFLoanApplication where vAutoEmployeeId = '"+employeeId+"' and iLoanStatus = '1' " ;
			...
			String active = " select * from tbPFLoanApplication where vAutoEmployeeId = '"+employeeId+"' and iLoanStatus = '1' and iSanctionStatus='1' " ;
						...
						if(!txtLoanPurpose.getValue().toString().equals(""))
						{
							if(!txtNoOfInstallment.getValue().toString().equals(""))
							{
								if(!txtSanctionAmount.getValue().toString().equals(""))
								{
									if(isUpdate)
									{
										String query="select * from tbPFLoanApplication where mLoanBalance=mSanctionAmount and vLoanNo = '"+txtFindLoanNo.getValue().toString()+"' and dApplicationDate = '"+txtFindDate.getValue()+"' ";
	...
	private void insertData()
	{
		...
		try
		{
			String insertQuery = "Insert into tbPFLoanApplication (vLoanNo,dApplicationDate,vAutoEmployeeId,vEmployeeId,"
	...
	private void updateData()
	{
		...
		try
		{
			updateQuery = "UPDATE tbPFLoanApplication set " +
