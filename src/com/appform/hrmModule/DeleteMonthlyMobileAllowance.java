...
public class DeleteMonthlyMobileAllowance extends Window
{
	...
	private ComboBox cmbMobileAllowanceMonth;
	...
	public DeleteMonthlyMobileAllowance(SessionBean sessionBean,String menuId) 
	{
		this.sessionBean= sessionBean;
		this.setCaption("DELETE MONTHLY MOBILE ALLAWANCE :: " +sessionBean.getCompany());
		...
		componentIni(true);
		cmbMobileAllowanceMonthData();
		setEventAction();
		authenticationCheck();
		focusEnter();
		cmbMobileAllowanceMonth.focus();
	}
	...
	{
		cmbMobileAllowanceMonth.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				cmbUnit.removeAllItems();
				if(cmbMobileAllowanceMonth.getValue()!=null)
				{
					vMonthName=sessionBean.dfMonthInt.format(cmbMobileAllowanceMonth.getValue());
					yearName=sessionBean.dfYear.format(cmbMobileAllowanceMonth.getValue());
					cmbUnitData();
				}
			}
		});
	...
	private void cmbMobileAllowanceMonthData()
	{
		cmbMobileAllowanceMonth.removeAllItems();
		...
		try
		{
			String query="select distinct cast(DATENAME(MM, dDate)as varchar)vMonth,YEAR(dDate)vYear,DATEADD(s,-1,DATEADD(mm, DATEDIFF(m,0,dDate)+1,0))dDate "
					+ "from tbMonthlyMobileAllowance order by dDate desc";
			System.out.println("cmbMobileAllowanceMonthData: "+query);
			...
			for(Iterator <?> iter=list.iterator();iter.hasNext();)
			{
				...
				cmbMobileAllowanceMonth.addItem( element[2]);
				cmbMobileAllowanceMonth.setItemCaption( element[2], element[0].toString()+"-"+element[1].toString());
			}
		}
		catch(Exception exp)
		{
			showNotification("cmbMobileAllowanceMonthData",exp+"",Notification.TYPE_ERROR_MESSAGE);
		}
	...
	private void deleteData()
	{
		...
		query = "select vEmployeeId from tbMonthlyMobileAllowance where MONTH(dDate)='"+vMonthName+"' and YEAR(dDate)='"+yearName+"' "
						...
						if(clickCount==0)
						{
							...
							try
							{
								String queryOld="insert into tbUdMonthlyMobileAllowance(" +
										...
										+ "from tbMonthlyMobileAllowance where MONTH(dDate)='"+vMonthName+"' and YEAR(dDate)='"+yearName+"' "
										+ "and vUnitID like '"+Unit+"' and vDepartmentID like '"+department+"' "
										+ "and vSectionID like '"+section+"' and vEmployeeID like '"+employee+"' ";
								
								session.createSQLQuery(queryOld).executeUpdate();

								String deleteQuery="delete from tbMonthlyMobileAllowance where MONTH(dDate)='"+vMonthName+"' and YEAR(dDate)='"+yearName+"' "
										+ "and vUnitID like '"+Unit+"' and vDepartmentID like '"+department+"' "
										+ "and vSectionID like '"+section+"' and vEmployeeID like '"+employee+"'"; 
		...
		{
			showNotification("Warning!","Data not found for month "+dMonth.format(cmbMobileAllowanceMonth.getValue())+" ", Notification.TYPE_WARNING_MESSAGE);
			txtClear();
			componentIni(true);
		}
	}
	...
	private void cmbUnitData()
	{
		...
		try
		{
			String query="select distinct vUnitId,vUnitName from tbMonthlyMobileAllowance where MONTH(dDate)='"+vMonthName+"' "
	...
	private void cmbSectionData() 
	{
		...
		try
		{
			String query="select distinct vSectionId,vSectionName from tbMonthlyMobileAllowance "
	...
	private void cmbEmployeeNameDataAdd()
	{
		...
		try
		{
			String query="select vEmployeeId,vEmployeeName,vEmployeeCode from tbMonthlyMobileAllowance "
	...
	{
		allComp.add(cmbMobileAllowanceMonth);
	...
	{
		cmbMobileAllowanceMonth.setValue(null);
	...
	{
		cmbMobileAllowanceMonth.setEnabled(b);
	...
	private AbsoluteLayout buildMainLayout()
	{
		...
		// cmbMobileAllowanceMonth
		cmbMobileAllowanceMonth = new ComboBox();
		cmbMobileAllowanceMonth.setImmediate(true);
		cmbMobileAllowanceMonth.setWidth("150px");
		cmbMobileAllowanceMonth.setHeight("-1px");
		cmbMobileAllowanceMonth.setFilteringMode(ComboBox.FILTERINGMODE_CONTAINS);
		mainLayout.addComponent(new Label("Month :"), "top:40.0px;left:30.0px;");
		mainLayout.addComponent(cmbMobileAllowanceMonth, "top:38.0px;left:150.0px;");
