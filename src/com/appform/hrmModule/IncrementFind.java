...
public class IncrementFind extends Window
{
	...
	public IncrementFind(SessionBean sessionBean,TextField txtFindId,String frmName)
	{
		this.txtFindId = txtFindId;
		this.sessionBean=sessionBean;
		this.frmNam=frmName;
		this.setCaption("INCREMENT FIND WINDOW :: "+sessionBean.getCompany());
		this.center();
		this.setWidth("420px");
		...
		txtFindFilter.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(!txtFindFilter.getValue().toString().isEmpty())
				{
					if(frmNam.equals("INCREMENT"))
					{
						tableDataIncrement("%"+txtFindFilter.getValue().toString()+"%");
					}
				}
			}
		});
	...
	private void tableDataIncrement(String department)
	{
		Session session = SessionFactoryUtil.getInstance().openSession();
		session.beginTransaction();
		try
		{			
			String query =" select vIncrementId,vIncrementType from tbIncrementName where vIncrementType like '%'";
		...
		{
			showNotification("tableDataIncrement", ex.toString(), Notification.TYPE_ERROR_MESSAGE);
		}
		finally
		{
			session.close();
