...
public class BankInfo extends Window 
{
	...
	public BankInfo(SessionBean sessionBean, String menuId)
	{
		this.sessionBean=sessionBean;
		this.setResizable(false);
		this.setCaption("BANK INFORMATION :: "+sessionBean.getCompany());
		this.menuId = menuId;
	...
	public void btnAction()
	{
		cButton.btnNew.addListener( new Button.ClickListener() 
		{
			public void buttonClick(ClickEvent event) 
			{
				...
				txtBankID.setValue(selectHeadId());
				txtBankName.focus();
			}
		});
		...
		txtBankName.addListener(new ValueChangeListener()
		{
			public void valueChange(ValueChangeEvent event)
			{
				if(!txtBankName.getValue().toString().isEmpty())
				{
					if(duplicateDepartmentName() && !isFind)
					{
						showNotification("Warning!","Bank Name already exist.",Notification.TYPE_WARNING_MESSAGE);
	...
	private void formValidation()
	{
		...
		{
			showNotification("Warning!","Provide Bank Name.", Notification.TYPE_WARNING_MESSAGE);
			txtBankName.focus();
		}
	}
	...
	private boolean duplicateDepartmentName()
	{
		...
		if(!isUpdate)
		{
			...
			try
			{
				String query = "select * from tbBankName where bankName" +
						" like '"+txtBankName.getValue().toString()+"'";
	...
	{
		Window win = new BankInfoFindWindow(sessionBean, txtFindId);
	...
	private String selectHeadId()
	{
		...
		try
		{
			String query = " Select isnull(max(cast(vBankId as int)),0)+1 from tbBankName ";
	...
	private void findInitialise(String BankId) 
	{
		Session session = SessionFactoryUtil.getInstance().openSession();
		Transaction tx = session.beginTransaction();
		...
		try 
		{
			sql = "select vBankId,bankName from tbBankName Where vBankId = '"+BankId+"'";
			...
			if (led.iterator().hasNext()) 
			{
				...
				txtBankID.setValue(element[0].toString());
				txtBankName.setValue(element[1].toString());
			}
	...
	{
		if(!txtBankID.getValue().toString().isEmpty())
		{
			isUpdate = true;
			isFind = false;
			btnIni(false);
	...
	private void insertData()
	{
		...
		try
		{
			String Insert = " INSERT into tbBankName(vBankId,bankName,vUserName,vUserIp,dEntryTime) values(" +
					" '"+selectHeadId()+"'," +
					" '"+txtBankName.getValue().toString().trim()+"'," +
	...
	public boolean updateData()
	{
		...
		try
		{
			...
			// Main table update
			String updateUnit = "UPDATE tbBankName set" +
					" bankName = '"+txtBankName.getValue().toString().trim()+"'" +
					" where vBankId = '"+txtFindId.getValue().toString()+"' ";
	...
	{
		//txtBankID.setEnabled(!t);
		txtBankName.setEnabled(!t);
	}
	...
	private AbsoluteLayout buildMainLayout() 
	{
		...
		txtBankID = new TextRead();
		txtBankID.setImmediate(true);
		txtBankID.setWidth("60px");
		txtBankID.setHeight("24px");
		txtBankID.setEnabled(false);
		mainLayout.addComponent(txtBankID, "top:38.0px;left:180.5px;");

		lblCommon = new Label("Bank Name :");
		mainLayout.addComponent(lblCommon, "top:80.0px; left:50.0px;");

		txtBankName = new TextField();
		txtBankName.setImmediate(true);
		txtBankName.setWidth("320px");
		txtBankName.setHeight("-1px");
		mainLayout.addComponent(txtBankName, "top:78.0px; left:180.0px;");
